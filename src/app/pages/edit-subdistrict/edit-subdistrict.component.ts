...
import { LoopBackAuth,MunicipalityApi,SubdistrictApi,DistrictApi,ProvinceApi,RegionApi,User_mapApi }   from '../../../shared/loopback_sdk';

@Component({
  templateUrl: 'edit-subdistrict.component.html'
})
export class EditSubdistrictComponent {
	...
	constructor(private sdApi:SubdistrictApi,private regionApi:RegionApi,private provApi:ProvinceApi,private dApi:DistrictApi,private mApi:MunicipalityApi,private router: ActivatedRoute, private loopAuth:LoopBackAuth,private userMapApi:User_mapApi){
	...
	submit() {
		...
		if(this.Prvnc&&this.Distr&&this.cty&&this.namechkRes == true&&(this.subdistrictName.match(/^[a-zA-Z0-9 ]+$/) && (this.subdistrictName!=''))){
			...
			this.busy=this.sdApi.addEditSubdistrict(para,this.id).subscribe((Res)=>{
				this.result = "success";
				this.showDialog();
			},(userErr)=>{
				this.result = "Failed";	
	...
	ngOnInit(): void {
		this.busy=this.sdApi.getSubDistrict('','','','',this.id).subscribe(
			data=>{
				this.subdistrictName=data.result[0]['name'];
				this.subdistrictcode=data.result[0]['subdistrict_code'];
				this.defname = data.result[0]['name'];
				this.defCode = data.result[0]['subdistrict_code'];
				...
				this.cty = {name:data['result'][0]['municipality'],id:data['result'][0]['municipality_id']};
	...
	NameChange(name){
		...
		if(name.match(/^[a-zA-Z0-9 ]+$/) && (name!='')){
			...
			this.busy=this.sdApi.subDistrictNamechek(name).subscribe((Res)=>{
				if(Res.result!=''){
