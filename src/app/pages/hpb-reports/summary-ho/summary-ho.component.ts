...
@Component({
    templateUrl: 'summary-ho.component.html'
  })
export class SummaryHoComponent{
    busy:any;
    ...
    constructor(private regionApi:RegionApi, private userApi:App_usersApi, private reportSummaryApi:ReportSummaryApi, 
	...
	ngAfterViewInit(): void{
		/** TODO:
		 * Fixing Query for ReportSummaryApi getReportSummaryHo
		 */
		console.log('TODO:Fixing Query for ReportSummaryApi getReportSummaryHo');
		this.resetData();
	}
    ...
    searchData(){
        ...
        console.log('acId:'+acId+',regionId:'+regionId+',fromDate:'+this.uiTxtFromDate+',toDate:'+this.uiTxtToDate);
        if(this.monthErr == false){
            ...
            this.busy = this.reportSummaryApi.getReportSummaryHo(acId, regionId, fromDate, toDate).subscribe(
				data=>{
				...
				data=>{
					...
					for(var kk=0;kk<data.result.length;kk++){
						...
						if(kk==(data.result.length-1)){
							(this.exportData).push({'Purchase (x)':'','Number of HPB':total,'%':'100.0%'});
						}
					}
					this.exportExcelUtil.makeExcel(this.exportData,"hpbReportSummaryHo");
				},
				err=>{},
				()=>{}
			)
