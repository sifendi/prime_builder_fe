...
@Component({
  templateUrl: 'ac-scorecard.component.html'
})
export class AcScorecardComponent {
	busy:any;
	...
	searchData(){
		...
		if(this.monthErr == false){
			this.busy = this.mappingApi.getScorecardSummaryWebView(this.rolename,this.user_id,acName,districtName,this.month,curYear).subscribe(
				data=>{
	...
	downloadData(){
		...
		this.busy = this.mappingApi.getScorecardSummary(null,null,this.rolename,this.user_id,acName,districtName,this.month,curYear).subscribe(
			data=>{
