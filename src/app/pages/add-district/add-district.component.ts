...
export class AddDistrictComponent {
	...
	nameChange:any;
	constructor(private dApi:DistrictApi,private provinceApi:ProvinceApi, private regionApi:RegionApi, private loopAuth:LoopBackAuth,private userMapApi:User_mapApi, private router:Router) {
	...
	submit() {
		...
		if(this.distName&&this.Prvnc&&this.code.valid&&this.namechkRes == true&&(this.distName.match(/^[a-zA-Z0-9 ]+$/) && (this.distName!=''))){
			...
			this.busy = this.dApi.addEditDistrict(para).subscribe((Res)=>{
				this.result = "success";
				this.display = true;
				setTimeout(()=>{
