...
@Component({
  templateUrl: 'add-postalcode.component.html'
})
export class AddPostalCodeComponent {
	
	@ViewChild('postal_codeId') postal_code: NgModel;
	...
	rdssbdstrct:any;
	nameChange:any;
	constructor(private pcApi:Postal_codeApi, private regionApi:RegionApi,private sdApi:SubdistrictApi,private provApi:ProvinceApi,private mApi:MunicipalityApi,private dApi:DistrictApi, private loopAuth:LoopBackAuth,private router:Router,private userMapApi:User_mapApi){
	...
	submit() {
		...
		if(this.Prvnc&&this.Distr&&this.cty&&this.subdistrict_id&&this.namechkRes == true&&(this.postal_codeId.match(/^[0-9]+$/) && (this.postal_codeId!=''))){
			...
			this.busy=this.pcApi.addEditPostalcode(para).subscribe((Res)=>{
				this.result = "success";
				this.showDialog();
				setTimeout(()=>{
	...
	NameChange(name){
		...
		if(name.match(/^[0-9]+$/) && (name!='')){
			...
			this.nameChange = this.busy = this.pcApi.postalcodeNamechek(name).subscribe((Res)=>{
				if(Res.result!=''){
